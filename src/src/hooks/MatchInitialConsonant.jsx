export const isInitialConsonantMatch = (query, target) => {
  const reg = new RegExp(query.split("").map(pattern).join(".*?"), "i");
  const matches = reg.exec(target);

  // console.assert(matches, "matches is null");
...
const con2syl = Object.fromEntries(orderOffset.map(([syl, code]) => [syl, code]));
